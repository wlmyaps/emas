        ...
        complete: function(results) {
            ...
            masterData = results.data.map(row => {
                ...
                return {
                    ...
                    Terakhir: parseFloat(row.Terakhir.replace(/\./g, '').replace(',', '.')),
                    Pembukaan: parseFloat(row.Pembukaan.replace(/\./g, '').replace(',', '.')),
                    Tertinggi: parseFloat(row.Tertinggi.replace(/\./g, '').replace(',', '.')),
                    Terendah: parseFloat(row.Terendah.replace(/\./g, '').replace(',', '.')),
                    Perubahan: parseFloat(row['Perubahan%'].replace(',', '.'))
                };
            }).filter(row => !isNaN(row.Terakhir)); // Hapus baris yang tidak valid
            // Set nilai default input tanggal berdasarkan data
            if (masterData.length > 0) {
                const minDate = masterData[0].TanggalObj.toISOString().split('T')[0];
                const maxDate = masterData[masterData.length - 1].TanggalObj.toISOString().split('T')[0];
                document.getElementById('startDate').value = minDate;
                document.getElementById('endDate').value = maxDate;
                analyzeData(); // Panggil analisis pertama kali
            }
        },
        error: function(error) {
            console.error('Error memuat CSV:', error);
            alert('Gagal memuat data CSV. Periksa koneksi dan URL.');
        }
    });
}
...
function getDataByDateRange(startDateStr, endDateStr) {
    ...
    return masterData.filter(row => {
        return row.TanggalObj >= start && row.TanggalObj <= end;
    });
}
...
// Jangan lupa untuk menghubungkan tombol "Analisis Data" dengan fungsi ini
...
function displayStreakMetrics(data) {
    ...
    let sumDailyChanges = 0;

    for (let i = 0; i < data.length; i++) {
        const close = data[i].Terakhir;
        const open = data[i].Pembukaan;
        
        // Hitung perubahan persen harian untuk rata-rata
        if (i > 0) {
        ...
        // Tren Open (membandingkan Open hari ini dengan Close kemarin)
        if (i > 0) {
            const prevClose = data[i-1].Terakhir;
            if (open < prevClose) {
                currentOpenDown++;
    ...
    const avgChange = data.length > 1 ? (sumDailyChanges / (data.length - 1)).toFixed(2) : 0;
    ...
    `;
    document.getElementById('streak-metrics').innerHTML = html;
}
...
function displayVolatilityMetrics(data) {
    ...
    `;
    document.getElementById('volatility-metrics').innerHTML = html;
}
...
function displayGapMetrics(data) {
    ...
    for (let i = 1; i < data.length; i++) {
        const gap = data[i].Pembukaan - data[i-1].Terakhir;
        totalGap += gap;
        gapCount++;
    }
    ...
    `;
    document.getElementById('gap-metrics').innerHTML = html;
}

// ... (lanjutan script.js)

let priceChart = null; // Variabel global untuk menyimpan instance chart
...
function displayPriceChart(data) {
    ...
    // Buat chart baru dengan data harga
    priceChart = new Chart(ctx, {
        type: 'line',
        data: {
            labels: labels,
            datasets: [{
                label: 'Harga Penutupan (IDR/gram)',
                data: prices,
    ...
    // Tambahkan event listener untuk tombol ganti tampilan
    document.getElementById('viewPriceBtn').onclick = () => {
        if (priceChart) {
            priceChart.data.datasets[0].label = 'Harga Penutupan (IDR/gram)';
            priceChart.data.datasets[0].data = prices;
            priceChart.data.datasets[0].borderColor = 'rgb(44, 62, 102)';
            priceChart.options.plugins.tooltip.callbacks.label = (context) => `Rp ${context.raw.toLocaleString('id-ID')}`;
            priceChart.update();
        }
    };
    ...
    document.getElementById('viewChangeBtn').onclick = () => {
        if (priceChart) {
            priceChart.data.datasets[0].label = 'Perubahan Harian (%)';
            priceChart.data.datasets[0].data = changes;
            priceChart.data.datasets[0].borderColor = 'rgb(220, 38, 38)';
...
function displayDistributionMetrics(data) {
    ...
    // Buat 10 interval (bins) berdasarkan rentang harga
    const binCount = 10;
    ...
    // Cari zona dengan frekuensi tertinggi
...
function displayExtremeValues(data) {
    ...
    for (const row of data) {
        if (row.Terakhir > maxPrice) {
            maxPrice = row.Terakhir;
            maxDate = row.Tanggal;
        }
        if (row.Terakhir < minPrice) {
            minPrice = row.Terakhir;
            minDate = row.Tanggal;
        }
    }
    ...
    `;
    document.getElementById('extreme-values').innerHTML = html;
}
...
function displayDirectionalVolatility(data) {
    ...
    for (const row of data) {
        const spike = row.Tertinggi - row.Terendah;
        if (row.Perubahan > 0) { // Hari Naik (Hijau)
            upDaySpikes += spike;
            upDayCount++;
        } else if (row.Perubahan < 0) { // Hari Turun (Merah)
            downDaySpikes += spike;
            downDayCount++;
        }
        // Hari datar (Perubahan = 0) diabaikan
    }
